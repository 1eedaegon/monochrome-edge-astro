...
test.describe('Article Page', () => {
  ...
  test('series navigation shows for series articles', async ({ page }) => {
    ...
    const seriesStepper = page.locator('.series-stepper, [class*="series"]').first()
    const hasSeries = await seriesStepper.isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('series stepper renders through Monochrome Edge package', async ({ page }) => {
